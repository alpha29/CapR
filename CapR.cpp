...
{
  vector<double> bulge_probability;
  bulge_probability.resize(_seq_length, 0.0);
  vector<double> internal_probability;
  internal_probability.resize(_seq_length, 0.0);
  vector<double> hairpin_probability;
  hairpin_probability.resize(_seq_length, 0.0);
  vector<double> multi_probability;
  multi_probability.resize(_seq_length, 0.0);
  vector<double> exterior_probability;
  exterior_probability.resize(_seq_length, 0.0);
  vector<double> stem_probability;
  stem_probability.resize(_seq_length, 0.0);
...
double CapR::CalcMultiProbability(int x)
{
  ...
  if (flag == 1)
  {
    probability = exp(temp - _Alpha_outer[_seq_length]);
  }
  return (probability);
}
...
void CapR::CalcLogSumBulgeAndInternalProbability(vector<double> &bulge_probability, vector<double> &internal_probability)
{
  ...
  int type = 0;
  int type2 = 0;
  vector<bool> b_flag_array;
  b_flag_array.resize(_seq_length, 0);
  vector<bool> i_flag_array;
  i_flag_array.resize(_seq_length, 0);
...
double CapR::CalcDangleEnergy(int type, int a, int b)
{
  ...
  {
    if (a > 0)
      x += dangle5[type][_int_sequence[a]];
    if (b < _seq_length)
      x += dangle3[type][_int_sequence[b + 1]];
